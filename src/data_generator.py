...
class AudioGenerator():
    def __init__(self, step=10, window=20, max_freq=8000, mfcc_dim=13,
        minibatch_size=20, desc_file=None, spectrogram=True, max_duration=10.0, min_duration=0.5,
    ...
    def load_train_data(self, desc_file=None):
        self.load_metadata_from_desc_file(desc_file, 'train')
        self.fit_train()
        if self.sort_by_duration:
            self.sort_data_by_duration('train')
    def get_train_mean_sd(self, desc_file=None):
        self.load_metadata_from_desc_file(desc_file, 'train')
        self.fit_train()
        if self.sort_by_duration:
            self.sort_data_by_duration('train')
        return self.feats_mean,self.feats_std
    def load_train_labels(self, desc_file=None):
        self.load_metadata_from_desc_file(desc_file, 'train')
        if self.sort_by_duration:
            self.sort_data_by_duration('train')

    def load_validation_data(self, desc_file=None):
        self.load_metadata_from_desc_file(desc_file, 'validation')
        if self.sort_by_duration:
            self.sort_data_by_duration('valid')

    def load_test_data(self, desc_file=None):
    ...
    def fit_train(self, k_samples=100):
        ...
        if self.feats_mean is not None and self.feats_std is not None:
            print('Normalization params already initialized / calculated.')
            return
    ...
    def normalize(self, feature, eps=1e-14):
        ...
        if self.feats_mean is None or self.feats_std is None:
            raise Exception("Normalization Params not initialized/calculated.")
